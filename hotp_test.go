...
func TestHmacShaN1(t *testing.T) {
	for _, testValue := range hotpTestValues {
		res := hmacShaN(sha1.New, testValue.Secret, testValue.Count)
...
func TestHOTP(t *testing.T) {
	for _, testValue := range hotpTestValues {
		res := HOTP(testValue.Secret, testValue.Count, HOTPOptions{})
...
func TestHOTPDefaults(t *testing.T) {
	...
	resDefaults := HOTP(testValue.Secret, testValue.Count, HOTPOptions{})
	resCustom := HOTP(testValue.Secret, testValue.Count, HOTPOptions{
		Digits:    6,
		Algorithm: sha1.New,
	})
